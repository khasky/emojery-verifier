...
import { bytesToHex, checkHashChain, leafHashFromEntry, merkleRootFromLeaves } from "../transparency.mjs";
...
// /log/entries-shaped rows [1..treeSize] from the API (paged) or from the log repo's
// entries/ shards. The whole log is held in memory: the fold, the invariants and the
// archive replay each need the full set. Around a million leaves that is about a
// gigabyte of heap; past a few million, fold and verify from a streamed source instead.
export async function fetchEntries(api, repo, entriesMode, treeSize) {
  const rows = [];
