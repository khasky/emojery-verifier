...
| `served_counts` | **reported, never enforced.** The exact `total` the public badge carries for the `--counts-sample` largest targets, against the fold of the log for those targets - the one look at a number the log alone cannot account for. It is a note rather than a check because the fold stops at the audited checkpoint while the served count is live: every vote cast since is already counted and not yet in any leaf, and an un-react moves it the other way, so no threshold here would be sound. The default base is pinned to the same deployment as the log key, so a run with another `--pubkey` needs `--counts-base`; `--counts-sample 0` skips it. |
...
| `swh` | with `--swh`: see the Software Heritage section. Absent otherwise. |
...
The verifier reads `ots/latest.json`, `ots/<tree_size>.json` and `ots/<tree_size>.ots` from `--repo`. `latest.json` names the newest matured proof, and its age decides first: a missing file is a skip while the newest checkpoint is at most `--ots-max-lag-hours` (default 48) old and a failure once it is older, and a present proof whose signed `ts` is further than that behind the newest checkpoint's fails as a stalled maturation pipeline. The lag is judged against the live tip, never against the checkpoint an offline run stepped back to. The sidecar's `root_hash` must be a 64-hex digest (it is repo-controlled and, with `--ots-external`, becomes an argument of a spawned process), the sidecar must be a validly signed STH, and the `.ots` proof, parsed locally with no OpenTimestamps dependency, must commit that root in a Bitcoin block: the proof's Bitcoin attestation must equal the merkle root in the attested block header, and each header must double-SHA256 to the block id the explorer returned for its height, so the merkle root is read from bytes pinned to that block. A multi-calendar proof anchors in several blocks; the sidecar's `btc_block_height` must be one of them. The chain header-to-id-to-attestation is confirmed; Bitcoin's proof-of-work is not re-verified, so the check pairs with the explorer's best-chain view rather than standing in for a full node. `--btc-api` names any Esplora-compatible API (default `https://blockstream.info/api`).
...
The OTS codec in `src/ots-core.mjs` is the one implementation of the format in the system: the operator's Worker only submits digests and stores the calendars' raw responses, and the operator's scheduled upgrade job imports this file from a checkout to grow them into the proofs checked here.

## Software Heritage archive (`--swh`)

Software Heritage is an independently operated archive of source code. The operator asks it daily to re-archive the log repository; this check reads the archive's own record rather than anything the operator wrote, since a file in the log saying "archived" is a claim and the archive's visit list is the fact. With `--repo` on a `raw.githubusercontent.com` base, the verifier takes the repository's GitHub origin, lists its visits at `https://archive.softwareheritage.org/api/1/origin/<origin>/visits/`, and picks the newest with `status: full`. There must be one, it must be at most `--swh-max-age-days` (default 7) old, its snapshot must name a `refs/heads/main` head (or the head `HEAD` aliases to), and that commit must exist in the GitHub repository (`GET /repos/<owner>/<repo>/commits/<sha>` answering 200), so the archived history is a state this repository really had rather than some other history under the same name. An unreachable archive, an unreadable snapshot or a rate-limited GitHub lookup is a skip; only a missing, stale or foreign archive fails. `GITHUB_TOKEN` lifts the unauthenticated quota on the commit lookup.
